...
public class PhotoGridAdapter extends SelectableAdapter<PhotoGridAdapter.PhotoViewHolder, Media> {
    ...
    private void onItemClicked(PhotoViewHolder holder, Media media) {
        if (PickerManager.getInstance().getMaxCount() == 1) {
            ...
            /*Toast.makeText(context, "Only "+PickerManager.getInstance().getMaxCount()+ " phots can be selected", Toast.LENGTH_SHORT).show();*/
            Toast.makeText(context, "Maximum 5 document allowed only", Toast.LENGTH_SHORT).show();
        }
    }
